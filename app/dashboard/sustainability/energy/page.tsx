...
const METER_TYPE_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  ELEKTRIK: Zap,
  DOGALGAZ: Flame,
  LPG: Fuel,
  JENERATOR: Gauge,
...
const METER_TYPE_COLORS: Record<string, string> = {
  ...
  DOGALGAZ: 'bg-orange-100 text-orange-800',
  LPG: 'bg-red-100 text-red-800',
  JENERATOR: 'bg-gray-100 text-gray-800',
};
...
export default function EnergyPage() {
  ...
  const fetchMeters = async () => {
    try {
      const res = await fetch('/api/sustainability/meters?category=ENERJI');
      if (res.ok) {
  ...
  const chartData = readings
    ...
    .map(r => ({
      ...
      tüketim: r.value,
      anomali: r.isAnomalous ? r.value : null,
    }));
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Enerji Tüketim Yönetimi</h1>
            <p className="text-sm text-gray-500">Elektrik · Doğalgaz · LPG · Jeneratör</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => setShowReadingDialog(true)} disabled={!selectedMeter}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="space-y-2">
          ...
          {meters.length === 0 ? (
            ...
            meters.map(meter => {
              const Icon = METER_TYPE_ICONS[meter.type] || Zap;
              return (
                <Card
                  key={meter.id}
                  className={`cursor-pointer transition-all ${selectedMeter?.id === meter.id ? 'ring-2 ring-yellow-400 bg-yellow-50' : 'hover:bg-gray-50'}`}
                  ...
                  <CardContent className="py-3 px-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <Badge className={`text-xs ${METER_TYPE_COLORS[meter.type]}`}>
              ...
              <div className="grid grid-cols-3 gap-3">
                <Card>
                  <CardContent className="py-3">
                    ...
                    <p className="text-lg font-bold">{totalConsumption.toFixed(1)} {selectedMeter.unit}</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="py-3">
                    <p className="text-xs text-gray-500">Ort. Tüketim</p>
                    <p className="text-lg font-bold">{avgConsumption.toFixed(1)} {selectedMeter.unit}</p>
                  </CardContent>
                </Card>
                <Card className={anomalyCount > 0 ? 'border-red-200 bg-red-50' : ''}>
                  <CardContent className="py-3">
                    <p className="text-xs text-gray-500">Anomali</p>
                    <p className={`text-lg font-bold ${anomalyCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {anomalyCount} tespit
                    </p>
                  </CardContent>
                </Card>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Ör: Ana Elektrik Sayacı"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Tür *</Label>
                <Select value={meterForm.type} onValueChange={v => {
                  const units: Record<string, string> = { ELEKTRIK: 'kWh', DOGALGAZ: 'm3', LPG: 'litre', JENERATOR: 'litre' };
                  setMeterForm({ ...meterForm, type: v, unit: units[v] || 'kWh' });
                }}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="JENERATOR">Jeneratör</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label>Seviye</Label>
                <Select value={meterForm.level} onValueChange={v => setMeterForm({ ...meterForm, level: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Ör: Mutfak, Spa, Odalar Bloku..."
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
            ...
            <div>
              <Label>Notlar</Label>
              <Input value={meterForm.notes} onChange={e => setMeterForm({ ...meterForm, notes: e.target.value })} />
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label>Dönem</Label>
                <Select value={readingForm.period} onValueChange={v => setReadingForm({ ...readingForm, period: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddReading} disabled={!readingForm.value}>Kaydet</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
