...
export default function SWOTStudiesPage() {
  const router = useRouter();
  const [studies, setStudies] = useState<SWOTStudy[]>([]);
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/swot-studies');
      const data = await res.json();
      setStudies(data.studies || []);
      setStats(data.stats || { total: 0, draft: 0, active: 0, completed: 0 });
    } catch (error) {
      console.error('Error fetching SWOT studies:', error);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success('SWOT çalışması oluşturuldu');
        setIsCreateOpen(false);
        setFormData({ name: '', description: '', periodId: '', departmentId: '' });
        router.push(`/dashboard/strategy/swot/${study.id}`);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">SWOT Analizleri</h1>
          <p className="text-muted-foreground">Güçlü yönler, zayıf yönler, fırsatlar ve tehditler</p>
        </div>
        <div className="flex gap-2">
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => {
                const columns: ExportColumn[] = [
                  ...
                  { header: 'Güçlü Yön', key: 'strengths', width: 10 },
                  { header: 'Zayıf Yön', key: 'weaknesses', width: 10 },
                  { header: 'Fırsat', key: 'opportunities', width: 10 },
                  { header: 'Tehdit', key: 'threats', width: 10 },
                ...
                const data = filteredStudies.map(s => ({
                  ...
                  strengths: s.items?.filter((i: any) => i.type === 'STRENGTH').length || 0,
                  weaknesses: s.items?.filter((i: any) => i.type === 'WEAKNESS').length || 0,
                  opportunities: s.items?.filter((i: any) => i.type === 'OPPORTUNITY').length || 0,
                  threats: s.items?.filter((i: any) => i.type === 'THREAT').length || 0,
                  date: new Date(s.analysisDate).toLocaleDateString('tr-TR'),
                }));
                exportToExcel(data, columns, 'SWOT-Analizleri', 'SWOT Listesi');
              ...
              <DropdownMenuItem onClick={() => {
                const columns: ExportColumn[] = [
                  ...
                  { header: 'Dönem', key: 'period', width: 25 },
                  { header: 'Durum', key: 'status', width: 15 },
                  { header: 'S/W/O/T', key: 'counts', width: 15 },
                ];
                const data = filteredStudies.map(s => ({
                  code: s.code,
                  name: s.name,
                  period: s.period.name,
                  status: statusConfig[s.status]?.label || s.status,
                  counts: `${s.items?.filter((i: any) => i.type === 'STRENGTH').length || 0}/${s.items?.filter((i: any) => i.type === 'WEAKNESS').length || 0}/${s.items?.filter((i: any) => i.type === 'OPPORTUNITY').length || 0}/${s.items?.filter((i: any) => i.type === 'THREAT').length || 0}`,
                }));
                exportToPDF(data, columns, 'SWOT-Analizleri', 'SWOT Analizleri Listesi');
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Yeni SWOT Analizi Oluştur</DialogTitle>
            </DialogHeader>
      ...
      {filteredStudies.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground">Henüz SWOT analizi bulunmuyor</p>
          ...
          {filteredStudies.map((study) => {
            ...
            return (
              <Card
                ...
                onClick={() => router.push(`/dashboard/strategy/swot/${study.id}`)}
              >
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center">
                    <div className="bg-green-50 rounded p-2">
                      <p className="text-lg font-bold text-green-700">
                        {(study.items || []).filter(i => i.type === 'STRENGTH').length}
                      </p>
                      <p className="text-xs text-green-600">Güçlü</p>
                    </div>
                    <div className="bg-red-50 rounded p-2">
                      <p className="text-lg font-bold text-red-700">
                        {(study.items || []).filter(i => i.type === 'WEAKNESS').length}
                      </p>
                      <p className="text-xs text-red-600">Zayıf</p>
                    </div>
                    <div className="bg-blue-50 rounded p-2">
                      <p className="text-lg font-bold text-blue-700">
                        {(study.items || []).filter(i => i.type === 'OPPORTUNITY').length}
                      </p>
                      <p className="text-xs text-blue-600">Fırsat</p>
                    </div>
                    <div className="bg-orange-50 rounded p-2">
                      <p className="text-lg font-bold text-orange-700">
                        {(study.items || []).filter(i => i.type === 'THREAT').length}
                      </p>
                      <p className="text-xs text-orange-600">Tehdit</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">
                      {(study.strategies || []).length} strateji
                    </span>
                    <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/strategy/swot/${study.id}`)}>
