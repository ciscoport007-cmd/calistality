...
type KapakRow = Awaited<ReturnType<typeof prisma.kapakReport.findFirst>>;
type StatRow  = Awaited<ReturnType<typeof prisma.dailyStatistic.findFirst>>;
type RateRow  = Awaited<ReturnType<typeof prisma.exchangeRate.findFirst>>;

function buildDetailResponse(kapak: NonNullable<KapakRow>, statistic: StatRow, exchangeRate: RateRow) {
    ...
    data: {
      ...
      statistics: statistic
        ? {
            ...
            lyAvailRoomToday: statistic.lyAvailRoomToday, lyAvailRoomMTD: statistic.lyAvailRoomMTD,
            lyAvailRoomYTD: statistic.lyAvailRoomYTD,
            lySoldRoomToday: statistic.lySoldRoomToday, lySoldRoomMTD: statistic.lySoldRoomMTD,
            ...
            lyOutOfOrderToday: statistic.lyOutOfOrderToday, lyOutOfOrderMTD: statistic.lyOutOfOrderMTD,
...
// GET /api/finance/kapak?latest=true      → full kapak data for most recent day
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── Latest mode ──────────────────────────────────────────────────────────
    if (latestParam) {
      const latest = await prisma.kapakReport.findFirst({ orderBy: { reportDate: 'desc' }, select: { reportDate: true } });
      if (!latest) return NextResponse.json({ success: true, data: null });
      // Fall through to detail mode with the latest date
      ...
      const [kapak, statistic, exchangeRate] = await Promise.all([
        ...
        prisma.exchangeRate.findFirst({ where: { reportDate: { gte: dayStart, lte: dayEnd } } }),
      ]);
      if (!kapak) return NextResponse.json({ success: true, data: null });
      // reuse same response shape — inline to avoid duplication
      return buildDetailResponse(kapak, statistic, exchangeRate);
    }
