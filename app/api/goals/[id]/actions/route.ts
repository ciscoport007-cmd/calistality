...
// GET - Hedefe bağlı aksiyonları listele
...
export async function POST(
  ...
  try {
    ...
    const goal = await prisma.strategicGoal.findUnique({
      where: { id: goalId },
      include: { objective: { include: { period: true } } },
    });
    ...
    const year = new Date(goal.objective.period.startDate).getFullYear();
