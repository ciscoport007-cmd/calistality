...
export async function GET(request: NextRequest) {
  try {
    ...
    const studies = await prisma.sWOTStudy.findMany({
      where,
      include: {
    ...
    console.error('SWOT studies fetch error:', error);
    return NextResponse.json({ error: 'Sunucu hatası' }, { status: 500 });
  }
}

// Yeni SWOT çalışması oluştur
export async function POST(request: NextRequest) {
  try {
    ...
    const count = await prisma.sWOTStudy.count({
      where: { code: { startsWith: `SWOT-${year}` } },
    });
    const code = `SWOT-${year}-${String(count + 1).padStart(3, '0')}`;

    const study = await prisma.sWOTStudy.create({
      data: {
        code,
        name,
        description,
    ...
    console.error('SWOT study create error:', error);
    return NextResponse.json({ error: 'Sunucu hatası' }, { status: 500 });
  }
}
